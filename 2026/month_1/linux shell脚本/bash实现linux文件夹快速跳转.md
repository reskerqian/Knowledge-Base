...
### 注释掉功能，或者禁用功能
1.  `~/.bashrc` 在该文件下将原来添加的文件注释掉，注释方法采用下面的。
2.  方法使用“Here Document”技巧，这是在脚本中注释掉大段代码最干净的方法，不需要在每一行前面加 #。你只需要用 : << 'COMMENT' 和 COMMENT 将代码包裹起来：
原理：: 是 Shell 中的空命令（不做任何事），<< 'COMMENT' 会将其后的内容作为输入，直到遇到结尾的 COMMENT。因为输入没有被赋值给任何变量，所以这段代码实际上被“跳过”了。
3. 执行命令，unset -f cd
4. type cd 看输出是否是cd is a shell builtin ，若是这个注释完成。
